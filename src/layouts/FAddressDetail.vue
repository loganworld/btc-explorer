    ...
    <div class="f-address-detail">
        ...
        >
            <!-- <f-card style="margin: 32px auto 0 auto; margin-bottom: 0;">
                <h1 class="h2" style="margin-bottom: 0;">
                    Scheduled network upgrade in progress.
                </h1>
            </f-card> -->
                ...
                <f-card>
                    ...
                    <div class="row no-collapse">
                        <div class="col f-row-label">
                            {{ $t("view_address_detail.available") }}
                        </div>
                        <div class="col">
                            <div v-show="'available' in cAssets">
                                {{ toCoin(cAssets.available) }} {{ symbol }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row no-collapse">
                        ...
                        <div class="col">
                            <div v-show="'delegated' in cAssets">
                                <FCoinTokenValue :value="cAssets.delegated" />
                            </div>
                        </div>
                    </div>
                    <div class="row no-collapse">
        ...
        <f-card>
            ...
            <div class="row">
                <div class="col">
                    <div class="num-block">
                        <h2 class="h3">
                            {{ $t("view_address_detail.value_in_coin") }}
                        </h2>
                        <div class="num">
                            <span v-show="cAccount">{{
                                toCoin(cAccount ? cAccount.totalValue : 1)
                            }}</span>
                        </div>
                    </div>
                </div>
                <div class="col">
                    <div class="num-block">
                        <h2 class="h3">
                            {{ $t("view_address_detail.value_in_usd") }}
                        </h2>
                        <div class="num">
                            <span v-show="cAccount">{{
                                toUSD(cAccount ? cAccount.totalValue : 1)
        ...
        <!-- <div class="f-subsection">
            <h2 class="h1">
                ...
                <span v-if="cAssetItems.length" class="f-records-count"
                    >({{ cAssetItems.length }})</span
                >
            </h2>
    ...
    computed: {
                ...
                if (delegations && delegations.edges) {
                    delegations.edges.forEach(_edge => {
                        ...
                        assets.delegated += delegation
                            ? WEITo(delegation.amount)
                            : 0;
                        assets.pending_rewards +=
                            delegation && delegation.pendingRewards
                                ? WEITo(delegation.pendingRewards.amount)
                                : 0;
                        assets.claimed_rewards += delegation
                            ? WEITo(delegation.claimedReward)
                            : 0;
                    });
        ...
        cAssetItems() {
            ...
            if (cAccount) {
                items.push(
                    this.getAssetItem(
                        this.$t("view_address_detail.available"),
                        cAccount.balance
                    )
                );
                ...
                if (delegation) {
                    items.push(
                        this.getAssetItem(
                            this.$t("view_address_detail.delegated"),
                            delegation.amount
                        )
                    );
                    items.push(
                        this.getAssetItem(
                            this.$t("view_address_detail.pending_rewards"),
                            delegation.pendingRewards.amount
                        )
                    );
                    items.push(
                        this.getAssetItem(
                            this.$t("view_address_detail.claimed_rewards"),
                            delegation.claimedReward
                        )
                    );
                } else {
                    items.push(
